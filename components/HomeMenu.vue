...
<template>
  <NuxtLink :to="!!path ? `${baseUrl}${routesPrefix}${path}` : '/'" @click.stop="onClick"
    :class="['group', bgColor, 'flex-wrap-reverse bg-opacity-20 cursor-pointer overflow-hidden backdrop-blur-sm relative shadow-lg ring-1 ring-black/5 items-center hover:shadow-md hover:shadow-gray-300 hover:dark:bg-slate-800 hover:dark:highlight-white/5 hover:bg-gradient-to-r from-transparent via-gray-500/50 to-transparent']">
    <img v-if="icon"
    ...
    <div
      ...
      <span v-if="$slots['second-line']" class="text-slate-700 text-sm font-medium dark:text-slate-400">
