...
const { defineNuxtPlugin, useRequestHeaders } = app
...
function isMobileOrTablet(a: any) {
  return REGEX_MOBILE_OR_TABLET1.test(a) || REGEX_MOBILE_OR_TABLET2.test(a.substr(0, 4))
}

function isIos(a: any) {
  return /iPad|iPhone|iPod/.test(a)
}

function isAndroid(a: any) {
  return /android/i.test(a)
}

function isWindows(a: any) {
  return /Windows/.test(a)
}

function isMacOS(a: any) {
...
function extractDevices(userAgent = DEFAULT_USER_AGENT) {
  let mobile = null
  let mobileOrTablet = null
  let ios = null
  let android = null

  const headers: any = useRequestHeaders(['user-agent'])
  ...
  else if ('cf-device-type' in headers) { // Cloudflare
...
const makeFlags = () => {
  ...
  const { mobile, mobileOrTablet, ios, android, windows, macOS, isSafari, isFirefox, isEdge, isChrome, isSamsung, isCrawler } = extractDevices(userAgent)
  return {
    extractDevices,
    userAgent,
    isMobile: mobile,
...
function getDevice(nuxtApp: any) {
  const flags = makeFlags()
  const device = reactive(flags)
  ...
  if (process.server) {
    ...
    if (REFRESH_ON_RESIZE && typeof window !== 'undefined') {
      window.addEventListener('resize', () => {
        setTimeout(() => {
          const newFlags: any = makeFlags()
          for (const key in newFlags)
            nuxtApp.$device[key] = newFlags[key]
        }, 50)
      })
    }
  }
